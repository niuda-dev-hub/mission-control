...
export function DiscoverAgentsModal({ onClose, workspaceId }: DiscoverAgentsModalProps) {
  const t = useTranslations('discoverAgents');
  ...
  const discover = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error || t('errors.discover', { status: res.status }));
        return;
      }
      const data = await res.json();
      setAgents(data.agents || []);
    } catch (err) {
      setError(t('errors.connect'));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        setError(data.error || t('errors.import'));
        return;
      }
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-3 sm:p-4">
      <div className="bg-mc-bg-secondary border border-mc-border rounded-t-xl sm:rounded-lg w-full max-w-2xl max-h-[88vh] sm:max-h-[80vh] flex flex-col pb-[env(safe-area-inset-bottom)] sm:pb-0">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading && (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="text-mc-text-secondary">{t('loading')}</span>
            </div>
          )}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs text-mc-text-secondary mt-0.5">
                          {agent.model && <span>{t('labels.model', { model: agent.model })}</span>}
                          {agent.channel && <span>{t('labels.channel', { channel: agent.channel })}</span>}
                          {agent.status && <span>{t('labels.status', { status: agent.status })}</span>}
                          <span className="text-mc-text-secondary/60">{t('labels.id', { id: agent.id })}</span>
                        </div>
                      </div>
                    </div>
                  );
        ...
        <div className="flex items-center justify-between p-4 border-t border-mc-border">
          <span className="text-sm text-mc-text-secondary">
            {selectedIds.size > 0 ? t('selected', { count: selectedIds.size }) : t('selectHint')}
          </span>
          <div className="flex flex-wrap gap-2">
            <button
              onClick={onClose}
            ...
            >
              {importResult ? t('actions.done') : t('actions.cancel')}
            </button>
            <button
            ...
            >
              {importing ? (
                ...
                <>
                  ...
                  {t('actions.import', { count: selectedIds.size })}
                </>
              )}
            </button>
          </div>
