    ...
    return (
      <div className="min-h-screen bg-mc-bg flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-mc-text-secondary">{t('loadingDashboard')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-mc-bg pb-[calc(1rem+env(safe-area-inset-bottom))]">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-lg sm:text-xl font-semibold truncate">{t('dashboardTitle')}</h1>
          ...
          <div className="flex items-center gap-2">
            <div className={`px-2.5 min-h-11 rounded-lg border text-xs flex items-center gap-2 ${sseConnected ? 'text-mc-accent-green border-mc-accent-green/40 bg-mc-accent-green/10' : 'text-mc-accent-yellow border-mc-accent-yellow/40 bg-mc-accent-yellow/10'}`}>
              ...
              {sseConnected ? t('liveBadge') : t('fallbackBadge')}
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
        <section className={`grid ${isPortrait ? 'grid-cols-2' : 'grid-cols-4'} gap-3`}>
          <MetricCard label={t('agents')} value={String(agents.length)} />
          <MetricCard label={t('working')} value={String(agents.filter((a) => a.status === 'working').length)} />
          <MetricCard label={t('blocked')} value={String(blockedAgentIds.size)} />
        ...
        <section className="bg-mc-bg-secondary border border-mc-border rounded-xl p-4">
          ...
          {nowWorking.length === 0 ? (
            <div className="text-sm text-mc-text-secondary">{t('noneWorking')}</div>
          ) : (
        ...
        <section>
          <div className="flex flex-wrap items-center gap-2 mb-3">
            ...
            {(['all', 'working', 'blocked', 'idle'] as ActivityFilter[]).map((tab) => (
              ...
              >
                {tab === 'all' ? t('all') : tab === 'working' ? t('working') : tab === 'blocked' ? t('blocked') : t('idle')}
              </button>
            ))}
          </div>
                ...
                <article key={agent.id} className="bg-mc-bg-secondary border border-mc-border rounded-xl p-4">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <div className="text-[11px] text-mc-text-secondary mt-1">{t('updatedPrefix')} {formatDistanceToNow(new Date(agent.updated_at), { addSuffix: true })}</div>
                    </div>
                  </div>
